...
**Last Updated:** 2025-11-11 (Session 13)
**Project:** CoachPro - Online Coaching Platform
**Tech Stack:** React 18 + Vite, Material-UI v6, Supabase, date-fns, Lucide React (icons)

**⚠️ KRITICKÉ - AUTH TROUBLESHOOTING:**
- Nová dokumentace: `docs/TROUBLESHOOTING_AUTH.md`
- Musíš ji přečíst při problémech s autentizací nebo materiály!
...
**⚠️ KRITICKÉ - AUTH_USER_ID:**
- **NIKDY** nepřepisuj `is_admin` bez kontroly (viz storage.js:85-87)
- **VŽDY** ukládej `auth_user_id` při přihlášení (AdminLogin, Tester, OAuth)
- **RLS vyžaduje** `auth_user_id` v `coachpro_coaches` tabulce
- Pokud `auth_user_id = NULL` → RLS blokuje přístup k materiálům
...
## 🔐 Autentizace - Důležité Poznatky (Session #13)

### Typy Přihlášení v CoachPro

| Typ | auth_user_id | Tabulky | RLS Funguje? |
|-----|--------------|---------|--------------|
| **Google OAuth** | ✅ Má | auth.users, coachpro_coaches, testers | ✅ Ano |
| **Email + Heslo (Admin)** | ✅ Má | auth.users, coachpro_coaches | ✅ Ano |
| **Access kód (starý)** | ❌ NULL | testers, coachpro_coaches | ❌ Ne |
| **Access kód (nový - TODO)** | ✅ Má | auth.users, testers, coachpro_coaches | ✅ Ano |

### Kritické Soubory pro Auth

**1. `src/modules/coach/utils/storage.js:85-87`**
```javascript
...
**2. `src/modules/coach/pages/AdminLogin.jsx:97-98`**
```javascript
// IMPORTANT: Save to Supabase to update auth_user_id
await saveCoach(adminUser); // MUSÍ být zavoláno!
```

**3. `src/modules/coach/pages/Tester.jsx:105-130`**
```javascript
...
**První kroky:**
1. Zkontroluj sessionStorage: `JSON.parse(sessionStorage.getItem('coachpro_currentUser'))`
2. Ověř auth_user_id v DB: `SELECT id, email, auth_user_id, is_admin FROM coachpro_coaches WHERE email = '...'`
3. Zkontroluj auth.users: `SELECT id, email FROM auth.users WHERE email = '...'`

**Detailní troubleshooting:**
→ Čti `docs/TROUBLESHOOTING_AUTH.md`
...
9. **Auth problém?** → `docs/TROUBLESHOOTING_AUTH.md`
10. **is_admin se přepsal?** → Zkontroluj `storage.js:saveCoach()`
