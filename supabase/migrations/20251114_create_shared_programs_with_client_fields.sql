-- Create shared programs table for individual paid sharing
-- Similar to coachpro_shared_materials, allows coaches to share programs with specific clients via code

CREATE TABLE IF NOT EXISTS coachpro_shared_programs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  program_id TEXT NOT NULL REFERENCES coachpro_programs(id) ON DELETE CASCADE,
...
-- Add comments
COMMENT ON TABLE coachpro_shared_programs IS 'Individual paid sharing of programs - one code per specific client with email verification';
...
COMMENT ON COLUMN coachpro_shared_programs.client_name IS 'Jméno konkrétní klientky (pro individuální placené sdílení)';
COMMENT ON COLUMN coachpro_shared_programs.client_email IS 'E-mail klientky pro ověření nároku na přístup (POVINNÝ pro placený obsah)';
...
CREATE INDEX IF NOT EXISTS idx_shared_programs_client_email ON coachpro_shared_programs(client_email);
CREATE INDEX IF NOT EXISTS idx_shared_programs_coach_email ON coachpro_shared_programs(coach_id, client_email);
