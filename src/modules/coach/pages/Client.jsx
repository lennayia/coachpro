...
const Client = () => {
  ...
  // OAuth user - minimal check (no auto-redirect)
      ...
      if (detectedType === 'program') {
        ...
        if (!client) {
          ...
          client = {
            ...
            auth_user_id: authUser?.id || null, // Link to OAuth user if logged in
          };
          await saveClient(client);
        } else if (authUser && !client.auth_user_id) {
          // Link existing client to OAuth user
          client.auth_user_id = authUser.id;
          await saveClient(client);
        }
